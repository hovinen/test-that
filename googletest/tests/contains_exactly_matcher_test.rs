...
#[test]
fn contains_exactly_matches_vector() -> Result<()> {
    let value = vec![1, 2, 3];
    verify_that!(value, contains_exactly![eq(1), eq(2), eq(3)])
}

#[test]
fn contains_exactly_omitted() -> Result<()> {
    let value = vec![1, 2, 3];
    verify_that!(value, {eq(3), eq(2), eq(1)})
}

#[test]
fn contains_exactly_matches_slice() -> Result<()> {
    let value = vec![1, 2, 3];
...
#[test]
fn contains_exactly_matches_vector_with_trailing_comma() -> Result<()> {
    let value = vec![1, 2, 3];
    verify_that!(value, contains_exactly![eq(1), eq(2), eq(3),])
}

#[test]
fn contains_exactly_matches_size() -> Result<()> {
    let value = vec![1, 2];
    verify_that!(value, not(contains_exactly![eq(1), eq(2), eq(3)]))
}
...
#[test]
fn contains_exactly_description_mismatch() -> Result<()> {
    let result = verify_that!(vec![1, 4, 3], contains_exactly![eq(1), eq(2), eq(3)]);
    verify_that!(
        result,
...
#[test]
fn contains_exactly_matches_unordered() -> Result<()> {
    let value = vec![1, 2];
    verify_that!(value, contains_exactly![eq(2), eq(1)])
}

#[test]
fn contains_exactly_matches_unordered_with_repetition() -> Result<()> {
    let value = vec![1, 2, 1, 2, 1];
    verify_that!(value, contains_exactly![eq(1), eq(1), eq(1), eq(2), eq(2)])
}

#[test]
fn contains_exactly_explains_mismatch_due_to_wrong_size() -> Result<()> {
    verify_that!(
        contains_exactly![eq(2), eq(3), eq(4)].explain_match(&vec![2, 3]),
        displays_as(eq("which has size 2 (expected 3)"))
    )
}

#[test]
fn contains_exactly_description_no_full_match() -> Result<()> {
    verify_that!(
        contains_exactly![eq(1), eq(2), eq(2)].explain_match(&vec![1, 1, 2]),
...
#[test]
fn contains_exactly_unmatchable_expected_description_mismatch() -> Result<()> {
    verify_that!(
        contains_exactly![eq(1), eq(2), eq(3)].explain_match(&vec![1, 1, 3]),
        displays_as(eq("which has no element matching the expected element #1"))
    )
}

#[test]
fn contains_exactly_unmatchable_actual_description_mismatch() -> Result<()> {
    verify_that!(
        contains_exactly![eq(1), eq(1), eq(3)].explain_match(&vec![1, 2, 3]),
