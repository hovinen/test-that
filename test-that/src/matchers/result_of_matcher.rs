...
macro_rules! __result_of_internal {
    ...
    (|$param:ident: $type:ty| $body:expr, $matcher:expr $(,)?) => {{
        $crate::matchers::__internal::ResultOfMatcher::new(
            ...
            $matcher,
            |$param: $type, matcher| {
                $crate::matchers::__internal::apply_matcher(&$body, matcher)
            },
            ...
            |$param: $type, definition, matcher| {
                ::core::convert::Into::into($crate::__alloc::format!(
                    ...
                    definition,
                    &$body,
                    $crate::matchers::__internal::explain_matcher(&$body, matcher)
                ))
            },
        )
    }};
...
pub mod __internal {
    ...
    /// Applies `matcher` to `value`.
    ///
    /// This exists so that the closure generated by `result_of!` names the
    /// value before the matcher, rather than calling
    /// [`Matcher::matches`][crate::matcher::Matcher::matches] directly. The
    /// value's type is then resolved from its own argument when
    /// `InnerMatcher: Matcher<ActualT>` is registered, instead of both being
    /// left to inference together.
    ///
    /// Without this, that type stays an inference variable while the bound is
    /// solved. The solver may then diverge in the presence of a recursive
    /// blanket implementation of a trait the matcher requires, reporting
    /// `E0275`.
    ///
    /// **For internal use only. API stability is not guaranteed!**
    #[doc(hidden)]
    pub fn apply_matcher<ActualT: Debug + ?Sized, InnerMatcher: Matcher<ActualT> + ?Sized>(
        value: &ActualT,
    ...
    /// Explains the match of `matcher` against `value`.
    ///
    /// The argument order matters for the same reason as in [`apply_matcher`].
    ///
    /// **For internal use only. API stability is not guaranteed!**
    #[doc(hidden)]
    pub fn explain_matcher<ActualT: Debug + ?Sized, InnerMatcher: Matcher<ActualT> + ?Sized>(
        value: &ActualT,
        matcher: &InnerMatcher,
    ) -> Description {
